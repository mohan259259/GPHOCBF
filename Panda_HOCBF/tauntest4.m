...
function tau = safe_control(t, x)
    ...
    % ========== 下面为 PTCBF 部分 ==========
    % 定义当前时间用于PTCBF（这里直接用 t，即可根据需要替换为 t_current_ptsf）
    t_current_ptsf = t;
    % 判断 phi：若安全，则 phi=1, phi_dot=0，否则使用 base 中定义的 phi 和 phi_dot
    if h1 >= 0
        phi_val = 1;
        phi_dot_val = 0;
    else
        phi_val = evalin('base', 'phi')(t_current_ptsf);
        phi_dot_val = evalin('base', 'phi_dot')(t_current_ptsf);
    end
    % 可以记录 phi 值（如需记录，请自行保存）
    % 定义 h2 = h1_dot + alpha1 * phi_val * h1
    h2 = h1_dot + alpha1 * phi_val * h1;
    % 定义 D1 根据你提供的 PTcbf 公式：
    D1 = -2*(dp'*dp) + alpha1*phi_dot_val*h1 + (alpha1+alpha2)*phi_val*h1_dot + alpha1*alpha2*phi_val^2*h1;
    % 构造 QP 约束：原代码中采用
    %      -2*(p-p0)'*Jp*tau >= 2*(dp'*dp + (p-p0)'*(Jp_dot*qdot)) -
    %      (alpha1+alpha2)*h1_dot - alpha1*alpha2*h1.
    % 现在我们用PTCBF中的 D1，令约束：
    %      -2*(p-p0)'*Jp*tau >= D1,
    % 或 equivalently,
    %       2*(p-p0)'*Jp*tau <= -D1.
    A = 2*(p-p0_local)'*Jp;  % 1x7
    b = -D1;
    % ========== 结束 PTCBF 部分 ==========
