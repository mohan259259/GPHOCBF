...
if (id > -1)
    ...
    alpha1 = 5;
    alpha2 = 30;
    ub = [1; 1; 1; 1; 1; 1; 1] * 80; % 跟实际的控制器力矩约束有关
    lb = -ub;
    ...
    while vrep.simxGetConnectionId(id) ~= -1
        ...
        %% 修改CBF
        if CBF_switch
            J = Panda_get_Jacobian(q,l1,l2,l3,d1,d2,lee);% 获取Jacobian 需要在vrep接口中查找
            Jdot = Panda_get_Jacobian(q,l1,l2,l3,d1,d2,l22);% 获取Jacobian导数，同样需要在vrep接口中查找
            [p1, pdot1, ~,~] = Panda_p_related(q, qdot, l1, l2, l3, d1, d2, lee);
            nonlinear = InverseDynamics(q, qdot, transpose(qddotDesired(i, :)), g, ...
                zeros(6, 1), Mlist, Glist, Slist);
            tau_norm = MassMatrix(q, Mlist, Glist, Slist) ...
                * (Kp * e + Kd * edot) + nonlinear;
            h1 = norm(p1 - p0)^2 - r^2;
            X = transpose(p1 - p0);
            Xdot = transpose(pdot1);
            if h1<=0
                % tau quadprog
                H = diag([2, 2, 2, 2, 2, 2, 2]);
                a = -2*X*J*inv(Mlist);
                b = alpha2*(2*X*J*qdot+alpha1*(norm(p1 - p0)^2 - r^2)+alpha1*2*X*J*qdot+2*X*Jdot*qdot+2*Xdot*J*qdot)-2*X*J*inv(Mlist)*(Clist*qdot+Glist*q);
