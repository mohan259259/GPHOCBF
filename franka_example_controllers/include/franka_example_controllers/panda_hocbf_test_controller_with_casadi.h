...
class PandaHocbfTestController : public controller_interface::MultiInterfaceController<
 ...
 public:
  ...
  struct CBF_Params {
    ...
    casadi::DM H;    // 权重矩阵（对角阵，均衡各关节力矩）
    casadi::DM f;    // 目标函数线性项，编码期望动力学
    Vector7d f_vec;  // 目标函数线性项，编码期望动力学
    casadi::DM A;    // 线性不等式约束矩阵
    Vector7d A_vec;  // 线性不等式约束矩阵
    casadi::DM b;    // 线性不等式约束向量
    double b_value;  // 线性不等式约束向量
    casadi::DM lbU;  // lower bound
    casadi::DM ubU;  // upper bound
    ...
    std::vector<double> output_vec;  // 输出向量
  ...
  // 用于线程安全
  casadi_opt casadi_cbf;
  std::atomic<bool> casadi_opt_startup;
