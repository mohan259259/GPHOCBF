...
//   action_init.qend =
//       (PandaHocbfTestController::Vector7d() << 0, 0, 0, -1 * M_PI_2, 0, M_PI_2, M_PI_4).finished();
...
//         // 计算二次规划目标函数的一次项向量
//         cbf_params.f_vec = -2 * (panda_data.M * current_state.qddot_norm + current_state.nonlinear);

//         {
//           std::lock_guard<std::mutex> lock(opt_mutex_);
//           cbf_params.f = casadi::DM::vertcat(
//               {cbf_params.f_vec[0], cbf_params.f_vec[1], cbf_params.f_vec[2], cbf_params.f_vec[3],
...
//         cbf_params.phi1 =
//             2 * cbf_params.C1.transpose() * current_state.pdot + cbf_params.alpha1 * cbf_params.h1;
...
//   cbf_params.alpha2 = 4;
    cbf_params.alpha1 = 5;
    cbf_params.alpha2 = 30;
  //   cbf_params.T_pre = 0.2;  //规定安全时间
  cbf_params.T_pre = 5;  //规定安全时间
  cbf_params.t0_val = 0;  //仿真开始时刻
                          //   cbf_params.c_gain = 2;
  cbf_params.c_gain = 0.002;
...
void PandaHocbfTestController::update(const ros::Time& /*time*/, const ros::Duration& period) {
  ...
  if (current_step < qDesired_total.size() - 1) {
    ...
    if (cbf_params.CBF_switch) {
      ...
      // 新增计算当前时间
      double t_current = current_step * dt;

      // 计算安全距离
      //   current_state.h_0 =
      //       (current_state.p - obstacle_position).squaredNorm() - std::pow(5 * obstacle_radius,
      //       2);
      // 新增计算h0
      current_state.h_0 =
          std::pow(5 * obstacle_radius, 2) - (current_state.p - obstacle_position).squaredNorm();
      ...
      if (current_state.h_0 <= 0) {
        ...
        // 新增f=0
        // cbf_params.f_vec = PandaHocbfTestController::Vector7d::Ones();
        cbf_params.f_vec = PandaHocbfTestController::Vector7d::Zero();
        {
          std::lock_guard<std::mutex> lock(opt_mutex_);
          cbf_params.f = casadi::DM::vertcat(
              {cbf_params.f_vec[0], cbf_params.f_vec[1], cbf_params.f_vec[2], cbf_params.f_vec[3],
        ...
        // 新增
        // 计算phi相关参数
        double phi_val = phi(t_current, cbf_params.T_pre, cbf_params.t0_val, cbf_params.c_gain);
        double phi_dot_val =
            phi_dot(t_current, cbf_params.T_pre, cbf_params.t0_val, cbf_params.c_gain);
        ...
        // 计算导数项
        cbf_params.C1 = -2 * (current_state.p - obstacle_position).transpose();

        double h1_dot = cbf_params.C1 * current_state.pdot;
        double h2 = h1_dot + cbf_params.alpha1 * phi_val * current_state.h_0;
        // 构造QP约束
        // cbf_params.D1 =
        //     -2 * current_state.pdot.squaredNorm() +
        //     cbf_params.alpha1 * phi_dot_val * current_state.h_0 +
        //     (cbf_params.alpha1 + cbf_params.alpha2) * phi_val * h1_dot +
        //     cbf_params.alpha1 * cbf_params.alpha2 * std::pow(phi_val, 2) * current_state.h_0;

        cbf_params.phi1 =
            2 * cbf_params.C1  * current_state.pdot + cbf_params.alpha1 * current_state.h_0;

        cbf_params.D1 = -2 *current_state.pdot.squaredNorm() +
                        0.5 * (cbf_params.alpha1 + cbf_params.alpha2) * cbf_params.phi1;

        // cbf_params.D1 = -2 * current_state.pdot.squaredNorm()+
        //     cbf_params.alpha1* current_state.h_0;

        // cbf_params.D1 = -0.1;

        std::cout << "D1_term0 = " << -2 * current_state.pdot.squaredNorm() << std::endl;
        std::cout << "D1_term1 = " << cbf_params.alpha1 * phi_dot_val * current_state.h_0
                  << std::endl;
        std::cout << "D1_term2 = " << (cbf_params.alpha1 + cbf_params.alpha2) * phi_val * h1_dot
                  << std::endl;
        std::cout << "D1_term3 = "
                  << cbf_params.alpha1 * cbf_params.alpha2 * std::pow(phi_val, 2) *
                         current_state.h_0
                  << std::endl;

        cbf_params.A_vec =
            ((-1) * cbf_params.C1 * current_state.J_p * panda_data.M.inverse()).transpose();
        {
          std::lock_guard<std::mutex> lock(opt_mutex_);
          cbf_params.A = casadi::DM::horzcat(
              {cbf_params.A_vec[0], cbf_params.A_vec[1], cbf_params.A_vec[2], cbf_params.A_vec[3],
        ...
        // cbf_params.b_value =
        //     cbf_params.C1 * current_state.Jdot_qdot + cbf_params.D1 -
        //     cbf_params.C1 * current_state.J_p * panda_data.M.inverse() * current_state.nonlinear;
        ...
        std::cout << "current_state.h_0 = " << current_state.h_0 << std::endl;
        std::cout << "phi_val = " << phi_val << std::endl;
        std::cout << "phi_dot_val = " << phi_dot_val << std::endl;
        std::cout << "cbf_params.C1 = " << cbf_params.C1 << std::endl;
        std::cout << "current_state.h_0 = " << current_state.h_0 << std::endl;
        std::cout << "h1_dot = " << h1_dot << std::endl;
        std::cout << "h2 = " << h2 << std::endl;
        std::cout << "cbf_params.D1 = " << cbf_params.D1 << std::endl;
        // std::cout << "current_state.J_p = " << current_state.J_p << std::endl;
        std::cout << "cbf_params.A_vec = " << cbf_params.A_vec << std::endl;
        std::cout << "cbf_params.b_value = " << cbf_params.b_value << std::endl;
        // 优化开启
        casadi_opt_startup = true;
...
void PandaHocbfTestController::thread_task() {
  while (true) {
    if (casadi_opt_startup) {
      //   std::cout << "casadi_opt_startup " << std::endl;
...
// 新增函数
double PandaHocbfTestController::phi(double t, double T_pre, double t0_val, double c_gain) {
  return (std::pow(T_pre, 2) +
          c_gain * std::pow((std::pow(t - t0_val, 2) - (t - t0_val) * T_pre), 2)) /
         std::pow(T_pre + t0_val - t, 2);
}

double PandaHocbfTestController::phi_dot(double t, double T_pre, double t0_val, double c_gain) {
  double numerator = c_gain * 2 * (std::pow(t - t0_val, 2) - (t - t0_val) * T_pre) *
                         (2 * (t - t0_val) - T_pre) * std::pow(T_pre + t0_val - t, 2) -
                     (std::pow(T_pre, 2) +
                      c_gain * std::pow((std::pow(t - t0_val, 2) - (t - t0_val) * T_pre), 2)) *
                         (-2 * (T_pre + t0_val - t));
  double denominator = std::pow(T_pre + t0_val - t, 4);
